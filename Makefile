...
build-dev:
	...
	APP_CONFIGURATION="--configuration=dev,delete-output-path,ignore-i18n,no-base-href,${APP_I18N_LOCALE_DEFAULT}-locale,${APP_I18N_LOCALE_DEFAULT}" EXEC_TARGET="build" make pnpm-run && APP_CONFIGURATION="--configuration=dev,ignore-i18n,localize" EXEC_TARGET="build" make pnpm-run && APP_CONFIGURATION="--configuration=dev,ignore-i18n,home-locale,home,home-base-href,home-config" EXEC_TARGET="build" make pnpm-run
build-production:
	...
	APP_CONFIGURATION="--configuration=production,delete-output-path,no-base-href,${APP_I18N_LOCALE_DEFAULT}-locale,${APP_I18N_LOCALE_DEFAULT}" pnpm run build && APP_CONFIGURATION="--configuration=production,localize" EXEC_TARGET="build" make pnpm-run && APP_CONFIGURATION="--configuration=production,home-locale,home,home-base-href,home-config" EXEC_TARGET="build" make pnpm-run
test-e2e:
