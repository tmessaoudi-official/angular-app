...
build-test:
	...
	APP_CONFIGURATION="--configuration=development,delete-output-path,ignore-i18n,no-base-href,${APP_I18N_LOCALE_DEFAULT}-locale,${APP_I18N_LOCALE_DEFAULT}${APP_CONFIGURATIOH_HOME_SUFFIX}" pnpm run build && APP_CONFIGURATION="--configuration=development,ignore-i18n,localize" pnpm run build && APP_CONFIGURATION="--configuration=development,ignore-i18n,home-locale-${APP_I18N_LOCALE_DEFAULT},home,home-base-href,home-config,dummy-env" pnpm run build
serve:
	NODE_ENV_FORCE=development APP_ENV_RUN_BUILD=true APP_ENV_FORCE_REBUILD=true APP_ENV_RUN_BUILD=true APP_ENV_FORCE_REBUILD=true  make load-env-js --ignore-errors --keep-going --warn-undefined-variables
	pnpm run serve
test:
	NODE_ENV_FORCE=test APP_ENV_RUN_BUILD=true APP_ENV_FORCE_REBUILD=true make load-env-js --ignore-errors --keep-going --warn-undefined-variables
	pnpm run test
...
test-e2e-selenium:
	APP_PROCESS_FORCE_ENV_INCLUDES="./.app.env/.env.test.e2e.selenium" make build-test
	APP_PROCESS_FORCE_ENV_INCLUDES="./.app.env/.env.test.e2e.selenium" NODE_ENV_FORCE=test APP_CONFIGURATION="--configuration=selenium" pnpm run e2e
