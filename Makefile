...
build-development:
	APP_NODE_ENV_INCLUDE=development APP_ENV_RUN_BUILD=true make load-env-js --ignore-errors --keep-going --warn-undefined-variables
...
build-production:
	APP_NODE_ENV_INCLUDE=production APP_ENV_RUN_BUILD=true  make load-env-js --ignore-errors --keep-going --warn-undefined-variables
...
build-test:
	APP_NODE_ENV_INCLUDE=test APP_ENV_RUN_BUILD=true make load-env-js --ignore-errors --keep-going --warn-undefined-variables
	APP_CONFIGURATION="--configuration=development,delete-output-path,ignore-i18n,no-base-href,${APP_I18N_LOCALE_DEFAULT}-locale,${APP_I18N_LOCALE_DEFAULT}${APP_LAUNCH_CONFIGURATIOH_HOME_SUFFIX}" pnpm run build && APP_CONFIGURATION="--configuration=development,ignore-i18n,localize" pnpm run build && APP_CONFIGURATION="--configuration=development,ignore-i18n,home-locale-${APP_I18N_LOCALE_DEFAULT},home,home-base-href,home-config,dummy-env" pnpm run build
serve:
	APP_NODE_ENV_INCLUDE=development APP_ENV_RUN_BUILD=true APP_ENV_RUN_BUILD=true APP_ENV_FORCE_REBUILD=true  make load-env-js --ignore-errors --keep-going --warn-undefined-variables
	pnpm run serve
test:
	APP_NODE_ENV_INCLUDE=test APP_ENV_RUN_BUILD=true make load-env-js --ignore-errors --keep-going --warn-undefined-variables
	pnpm run test
