...
export class EnvironmentVariableProcessors {
  ...
  public process(value: any|string|undefined|never, name: string|undefined = '', fail?: any): any|string|undefined|never {
    if (typeof value[0] !== 'undefined' && typeof value[0][0] !== 'undefined') {
      if (value[0][0] === 'environment.processors.json') {
        ...
        console.log(value);
        throw new Error('Environment variable processor \'' + value[0][0] + '\' does not exist');
      }
    }
  ...
  private processJson(value: string, name?: string, fail?: any, config?: any): object|null|undefined {
    let json: any;
    try {
      json = JSON.parse(value);
    } catch (exception) {
      if (processEnv.APP_DEBUG === 'true' || processEnv.APP_DEBUG === true) {
        console.warn(' ** Environment variable \'' + name + '\' contains invalid json');
        console.warn(exception);
        console.warn(fail);
        console.warn(config);
      }
      const canFail = config.find((configValue, configIndex, configArr) => configValue === 'fail=true' || configValue === 'fail');
      // eslint-disable-next-line max-len
      if (fail === true || (typeof config !== 'undefined' && typeof canFail !== 'undefined')) {
        console.log(canFail);
        console.log(fail);
        console.log(fail);
        throw new Error('Cannot parse environment variable \'' + name + '\'');
      }
  ...
  private processEmpty(value: any, name?: string, fail?: any, config?: any): any|null|undefined {
    if (value === 'undefined') {
      return undefined;
    } else if (value === 'null') {
      return null;
