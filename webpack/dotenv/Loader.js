...
const Loader = {
	load: function (fileName = `./.env`, included = ``) {
		...
		try {
			if (
				Object.keys(
					Loader.data.APP_ENV_FILES_ALL_LOADED.included
				).includes(fileName)
			) {
			...
			Loader.debug.status.debugFileLoad(
				fileName,
				`failure`,
				included,
				exception
	...
	debug: {
		status: {
			debugFileLoad: function (
				...
				if (
					!Object.keys(
						Loader.data.APP_ENV_FILES_ALL_LOADED.used
					).includes(fileName)
				) {
					Loader.data.APP_ENV_FILES_ALL_LOADED.used[fileName] = {
						status: status,
						from: parent,
						error: exception
					};
				...
				if (parent !== `` && parent !== false) {
					if (
						!Loader.data.APP_ENV_FILES_ALL_LOADED.used.includes(
							fileName
						)
					) {
						Loader.data.APP_ENV_FILES_ALL_LOADED.included[
		...
		canDebug: function () {
			return (
				...
				(!Loader.handlers.processors.boolean.isFalse(
					process.env.NODE_DEBUG_FORCE
				) ||
					typeof process.env.NODE_DEBUG_FORCE !== `string`)
			);
		}
	},
	include: function (includes, parentFile) {
		...
		if (typeof includes === `string` && includes !== ``) {
			includes.split(`,`).forEach(function (item) {
				...
				if (item === parentFile) {
					console.error(
						`Circular reference detected, file '` +
							item +
	...
	populate: function (data, fileName = ``) {
		...
		Object.values(data).forEach(function (item, index) {
			...
			if (keys[index] === `env_includes`) {
				...
				const handlers = Loader.handlers.getHandlers(item);
				if (
					...
					if (
						...
						item =
							...
							(typeof process.env[handlers.unHandledValue] !==
								`undefined` &&
							process.env[handlers.unHandledValue] !== null
								? process.env[handlers.unHandledValue]
								: `environment.processors.empty:` +
								  process.env[handlers.unHandledValue]);
					}
				}
				if (typeof item === `undefined` || item === null) {
					process.env[keys[index]] =
						`environment.processors.empty:` + item;
				} else if (Loader.handlers.processors.boolean.isBoolean(item)) {
					process.env[
						keys[index]
					] = Loader.handlers.processors.boolean.getValue(item);
				} else {
	...
	quote: {
		all: function () {
			Loader.data.APP_ENV_KEYS.forEach(function (item) {
		...
		processors: {
			boolean: {
				isBoolean: function (value) {
					return (
						Loader.handlers.processors.boolean.isTrue(value) ||
						Loader.handlers.processors.boolean.isFalse(value)
					);
				},
				getValue: function (value) {
					...
					throw new Error(
						`Trying to get boolean from : "` + value + `"`
					);
				},
	...
	run: function (nodeEnv = ``, force = false) {
		...
		if (!force) {
			try {
				if (fs.existsSync(`./.app.env/.env.local.build`)) {
					...
					Loader.log(
						`using generated file ./.app.env/.env.local.build`,
						`warn`
					);
					return;
				}
		...
		if (typeof process.env.APP_ENV_CONFIG_MISSABLE !== `undefined`) {
			JSON.parse(
				...
				if (
					typeof process.env[item] === `undefined` ||
					process.env[item] === `undefined` ||
					process.env[item] === null
				) {
					process.env[item] =
						`environment.processors.empty:` + process.env[item];
				}
			});
		}
		Loader.include(
			Loader.unquote.value(process.env[`process_env_includes`])
		);
			...
			function (item) {
				Loader.log(
					`            -- ` +
						item +
						` : ` +
						Loader.data.APP_ENV_FILES_ALL_LOADED.used[item].status +
						(Loader.data.APP_ENV_FILES_ALL_LOADED.used[item]
							.from !== false
							? ` , included by : ` +
							  Loader.data.APP_ENV_FILES_ALL_LOADED.used[item]
									.from
			...
			function (item) {
				Loader.log(
					`            -- ` +
						item +
						` : ` +
						Loader.data.APP_ENV_FILES_ALL_LOADED.included[item]
							.status +
						` , included by : ` +
						Loader.data.APP_ENV_FILES_ALL_LOADED.included[item]
							.from,
...
if (
	...
	fs.writeFile(`./.app.env/.env.local.build`, data, function (err) {
		...
		console.log(
			`Generated env created succussfully ./.app.env/.env.local.build`
		);
	});
}
